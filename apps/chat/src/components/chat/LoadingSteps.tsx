...
export function LoadingSteps({ step, language }: LoadingStepsProps) {
  ...
  return (
    <div className="flex items-center space-x-3 p-3 bg-muted/30 rounded-xl border w-fit text-sm animate-in fade-in zoom-in-95 duration-300">
      <Loader2 className="w-4 h-4 animate-spin text-primary" />
      <span className="text-primary font-medium animate-pulse">{currentLabel}</span>
    </div>
  );
}
